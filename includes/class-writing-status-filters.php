...
class WritingStatusFilters extends WritingStatusRenderer {
	...
	public function addCompletionFilterDropdown( $post_type ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only filter dropdown value, not a state-changing action.
	...
	public function filterPostsByCompletion( $query ) {
		...
		// Read-only filter dropdown values, not a state-changing action.
	...
	private function applyCompletionFilter( $query, &$filter_meta_query ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only filter dropdown value, not a state-changing action.
		$filter = isset( $_GET['writing_completion_filter'] ) ? sanitize_text_field( wp_unslash( $_GET['writing_completion_filter'] ) ) : '';
	...
	private function applyPriorityFilter( $query, &$filter_meta_query, $has_completion_filter ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only filter dropdown value, not a state-changing action.
		$priority_filter = isset( $_GET['writing_priority_filter'] ) ? sanitize_text_field( wp_unslash( $_GET['writing_priority_filter'] ) ) : '';
